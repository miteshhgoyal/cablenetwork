...
const Signin = () => {
    const { login, isAuthenticated, subscriptionStatus } = useAuth();
        ...
        const check = async () => {
            ...
            try {
                const r2 = await fetch(
                    'https://api.onlineiptvhub.com/api/health'
                );
    ...
    useEffect(() => {
        const getDeviceInfo = async () => {
            const info = {
                macAddress:
                    Device.modelId || Device.osBuildId || 'UNKNOWN_DEVICE',
    ...
    const handleSubmit = async (useCustomMac = false) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (
                    result.data?.canUseCustomMac &&
                    (result.code === 'MAC_INACTIVE' ||
                        result.code === 'SUBSCRIPTION_EXPIRED')
                ) {
                    setInactiveMessage(result.message);
                    setShowCustomMacModal(true);
                } else if (
                    result.code === 'CUSTOM_MAC_NOT_ACTIVE' ||
                    result.code === 'CUSTOM_MAC_EXPIRED'
                ) {
    ...
    // Single (mobile-style) layout in LANDSCAPE
    return (
        <View className="flex-1 bg-black">
                ...
                >
                    <View className="flex-1 justify-center px-6 py-8">
                        ...
                        {error ? (
                            <View className="mb-6 p-4 bg-red-900/30 rounded-xl border border-red-600">
                                <View className="flex-row items-start">
                                    ...
                                    />
                                    <Text
                                        className="text-red-400 text-sm ml-2 flex-1"
                                        style={{ lineHeight: 20 }}
                                    >
                                        {error}
                                    </Text>
                        ...
                        <TouchableOpacity
                            ...
                            disabled={isLoading}
                            className={`py-4 rounded-xl ${isLoading ? 'bg-gray-700' : 'bg-orange-500'
                                } shadow-lg`}
                            style={{ elevation: 5 }}
                        >
                            {isLoading ? (
                                    ...
                                    <View className="flex-row items-center">
                                        ...
                                        <View className="ml-3 flex-1">
                                            ...
                                            <Text className="text-gray-400 text-xs mt-0.5">
                                                Login with another device&apos;s active MAC
                                            </Text>
                                        </View>
                                    </View>
                                </View>
            ...
            {/* Custom MAC Modal (mobile-style, reused) */}
            <Modal
                animationType="slide"
                transparent
                    ...
                    >
                        <View className="items-center mb-6">
                            ...
                            <Text className="text-gray-400 text-sm mt-2 text-center">
                                Your device is not active. Login with another device&apos;s
                                active MAC address.
                            </Text>
                        </View>
                        ...
                        <View>
                            <TouchableOpacity
                                ...
                                className={`py-4 mb-3 rounded-xl ${!customMac.trim() || isLoading
                                        ? 'bg-gray-700'
                                        : 'bg-orange-500'
                                    }`}
                            >
