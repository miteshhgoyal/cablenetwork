...
import FastImage from 'react-native-fast-image'; // High-performance loader
import { SvgXml } from 'react-native-svg';
...
export default function Index() {
    ...
    const defaultChannelPlayedRef = useRef(false);
    ...
    const channelsByLanguage = useMemo(() => {
        ...
        const getDefaultPackageChannel = () => {
  if (!user?.package || !packagesList || !channelsByLanguage.length) return null;

  const pkg = packagesList.find(p => p._id === user.package);
  if (!pkg?.defaultChannelId?._id) return null;

  for (let catIdx = 0; catIdx < channelsByLanguage.length; catIdx++) {
    const chIdx = channelsByLanguage[catIdx].channels.findIndex(
      ch => ch._id === pkg.defaultChannelId._id
    );
    if (chIdx !== -1) {
      return {
        channel: channelsByLanguage[catIdx].channels[chIdx],
  ...
  return null;
    ...
    // Auto-start getDefaultPackageChannel channel on mount
useEffect(() => {
  if (defaultChannelPlayedRef.current) return;
  if (!channelsByLanguage.length || !packagesList?.length) return;

  const result = getDefaultPackageChannel();
  if (!result) return;

  const { channel, categoryIndex, channelIndex } = result;

  setFocusedCategory(categoryIndex);
  setFocusedChannelIndex(channelIndex);
  setIsCategoryFocused(true);

  handleChannelChange(channel);
  defaultChannelPlayedRef.current = true;
}, [channelsByLanguage, packagesList, user]);
    ...
    if (isCategoryFocused) {
        ...
        if (focusedCategory < channelsByLanguage.length - 1) {
            ...
            if (nextCategory && nextCategory.channels[0]) {
                if (!isCategoryFocused && nextCategory?.channels?.[0]) {
        ...
        if (isCategoryFocused) {
            ...
            if (focusedCategory > 0) {
                ...
                if (!isCategoryFocused && prevCategory?.channels?.[0]) {
        ...
        const handleChannelNumberInput = (digit) => {
            setChannelNumberInput(prev => {
                ...
                return newInput;
                                ...
                                // Enhanced RemoteChannelImage: if SVG fails, try converting to JPG and load that as fallback
                        // const RemoteChannelImage = ({
                        //     uri,
                        //     style,
                        //     resizeMode = 'contain',
                        ...
                        //     const [jpgUri, setJpgUri] = useState(null);
                        ...
                        //         // SVG validation: only treat as SVG if .svg and fetch returns valid SVG and Content-Type
                        //         const isSvgPath = uri.split('?')[0].toLowerCase().endsWith('.svg');
                        //         if (isSvgPath) {
                        //             setIsSvg(true);
                        //             let cancelled = false;
                        //             fetch(uri)
                        ...
                        //                     if (!cancelled) {
                        //                         if (
                        //                             (contentType.includes('svg') || text.includes('<svg')) &&
                        //                             text.trim().startsWith('<svg')
                        //                         ) {
                        //                             setSvgXml(text);
                        //                             setLoading(false);
                        //                         } else {
                        //                             // SVG fetch failed or not valid SVG, try to convert to JPG
                        //                             // Only attempt if running in Node.js (server-side) or if a backend endpoint is available
                        //                             // For demo: try to use a local conversion script if available
                        //                             if (typeof window === 'undefined') {
                        //                                 // Node.js: require and use svgToJpg.js
                        //                                 try {
                        //                                     const convertSvgToJpg = require('../scripts/svgToJpg');
                        //                                     const path = require('path');
                        //                                     const svgPath = uri;
                        //                                     convertSvgToJpg(svgPath)
                        //                                         .then(jpgPath => {
                        //                                             if (!cancelled) {
                        //                                                 setJpgUri(jpgPath);
                        //                                                 setTryJpg(true);
                        ...
                        //                                 }
                        //                             } else {
                        //                                 // On client, cannot convert SVG to JPG directly
                        //                                 setError(true);
                        //                                 setLoading(false);
                        //                             }
                        //                         }
                        ...
                        //         }
                        //         if (tryJpg && jpgUri) {
                        //             // Try to render the generated JPG
                        //             return (
                        //                 <View style={style}>
                        //                     <Image
                        //                         source={{ uri: jpgUri }}
                        ...
                        //                         </View>
                        //                     )}
                        //                     {error && <FallbackView style={style} icon={fallbackIcon} />}
                        //                 </View>
                        //             );
                        //         }
                        //         if (error) return <FallbackView style={style} icon={fallbackIcon} />;
...
const RemoteChannelImage = ({
  ...
  // 1. Handle Empty or Error State
  if (!uri || error) {
    return (
      <View style={[style, styles.centered, { backgroundColor: '#111' }]}>
        <Ionicons name={fallbackIcon} size={32} color="#6b7280" />
      </View>
    );
  }

  // 2. SVG logic (keep this if you need to fetch raw SVG code)
  const isSvg = uri.split('?')[0].toLowerCase().endsWith('.svg');
  if (isSvg) {
     // ... insert your existing SVG fetch logic here ...
  }

  // 3. Robust Wix/JPG handling using FastImage
  return (
    <View style={style}>
      <FastImage
        style={{ width: '100%', height: '100%' }}
        source={{
          uri: uri,
          // Wix tokens work better with explicit cache & priority settings
          priority: FastImage.priority.normal,
          cache: FastImage.cacheControl.immutable,
        }}
        resizeMode={FastImage.resizeMode[resizeMode] || FastImage.resizeMode.contain}
        onLoadStart={() => setLoading(true)}
        onLoadEnd={() => setLoading(false)}
      ...
      {loading && (
        <View style={[style, styles.loaderOverlay]}>
          <ActivityIndicator color="#f97316" />
        </View>
      )}
    </View>
...
useEffect(() => {
  return () => {
    debouncedHandleChannelChange.cancel();
  };
}, []);
                ...
                {selectedChannel && (
                    <View className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/70 to-transparent p-6">
                        <View className="flex-row items-center justify-between">
                            ...
                            <RemoteChannelImage
                                uri={selectedChannel.imageUrl || selectedChannel.logo}
                                style={{
                                    position: 'absolute',
                                    bottom: 16,
                                    right: 16,
