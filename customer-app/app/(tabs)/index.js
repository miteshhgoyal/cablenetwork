...
export default function ChannelsScreen() {
    ...
    const getUserPackage = () => {
        ...
        return packagesList.find(pkg => pkg.id === user.package);
    };
    ...
    const getRecommendedChannels = () => {
        ...
        return channels
            .filter(ch => ch.language?.name === selectedChannel.language?.name && ch.id !== selectedChannel.id)
            .slice(0, 15);
    };
    ...
    const analyzeStreamUrl = (url) => {
        ...
        if (urlLower.includes('youtube.com') || urlLower.includes('youtu.be')) {
            ...
            if (urlLower.includes('channel')) return { type: 'youtube-channel', isValid: true };
        ...
        if (url.match(/4/)) return { type: 'iptv', isValid: true };
    ...
    const getCurrentStreamUrl = (channel, useProxy) => {
        const baseUrl = useProxy && channel.proxyUrl ? channel.proxyUrl : channel.url;
        return {
            uri: baseUrl,
            headers: {
    ...
    useEffect(() => {
        if (selectedChannel && showPlayer) {
            ...
            if (type.type.startsWith('youtube')) {
                const newUrl = { uri: selectedChannel.url };
                ...
                const proxyUrl = getCurrentStreamUrl(selectedChannel, true);
                const directUrl = getCurrentStreamUrl(selectedChannel, false);
                const loadStream = async () => {
                    try {
                        await videoRef.current.loadAsync(proxyUrl, { shouldPlay: true });
                        setCurrentStreamUrl(JSON.stringify(proxyUrl));
                        setVideoLoading(false);
                        setVideoError(false);
                    } catch (error) {
                        try {
                            await videoRef.current.loadAsync(directUrl, { shouldPlay: true });
                            setCurrentStreamUrl(JSON.stringify(directUrl));
                            setVideoLoading(false);
                            setVideoError(false);
                        } catch (error) {
                            setVideoError(true);
                            setErrorMessage('Not playing');
                        }
                    }
                };
                loadStream();
        ...
        if (!type.isValid) {
            return (
                <View className="w-full bg-black items-center justify-center" style={{ height: 260 }}>
                    ...
                    <Text className="text-gray-400 text-center mt-2 px-4 text-sm">The provided URL format is not supported</Text>
                </View>
            );
        }
            ...
            <View className="w-full bg-black relative" style={{ height: 260 }}>
                ...
                {videoLoading && (
                    <View className="absolute inset-0 bg-black items-center justify-center z-20">
                        ...
                        <Text className="text-white mt-3 text-sm">Loading {type.type.toUpperCase()} stream...</Text>
                ...
                {videoError && (
                    <View className="absolute inset-0 bg-black items-center justify-center z-30">
                        ...
                        <Text className="text-gray-400 text-center mt-2 px-4 text-sm">{errorMessage}</Text>
                ...
                <Video
                    ...
                    ref={videoRef}
                    source={currentStreamUrl ? JSON.parse(currentStreamUrl) : null}
                    rate={1.0}
                    volume={1.0}
                    isMuted={false}
    ...
    const renderChannelItem = ({ item }) => {
        return (
            <TouchableOpacity className="bg-gray-800 rounded-lg mb-3 overflow-hidden active:bg-gray-700" onPress={() => handleChannelPress(item)}>
                <View className="relative" style={{ height: 120 }}>
                    ...
                    <View className="absolute top-2 left-2 bg-black80 px-2.5 py-1 rounded-md">
                ...
                <View className="p-3">
                    ...
                    <Text className="text-gray-400 text-sm mt-1">{item.language?.name || 'Unknown'} {item.genre?.name || 'General'}</Text>
                </View>
            </TouchableOpacity>
        );
    };
        ...
        return (
            <SafeAreaView className="flex-1 bg-black">
                ...
                <View className="flex-1 items-center justify-center px-6">
                    ...
                    <Text className="text-white text-xl font-semibold mt-4 text-center">No Channels Available</Text>
                    <Text className="text-gray-400 mt-2 text-center">Please check your subscription or contact support</Text>
    ...
    return (
        <SafeAreaView className="flex-1 bg-black">
            ...
            />
            <Modal visible={showPlayer} animationType="slide" onRequestClose={() => { setShowPlayer(false); setSelectedChannel(null); }}>
                <SafeAreaView className="flex-1 bg-black">
                    <StatusBar barStyle="light-content" />
                    <View className="flex-row items-center justify-between px-4 py-3 bg-gray-900 border-b border-gray-800">
                        <TouchableOpacity onPress={() => { setShowPlayer(false); setSelectedChannel(null); }}>
                            <Ionicons name="arrow-back" size={24} color="white" />
                        </TouchableOpacity>
                        <View className="flex-1 mx-4">
                            <Text className="text-white font-bold text-lg numberOfLines={1}">{selectedChannel?.name} Channel</Text>
                            <Text className="text-gray-400 text-sm">LCN {selectedChannel?.lcn} {selectedChannel?.language?.name}</Text>
                        </View>
                        <View className="flex-row items-center bg-gray-800 px-3 py-2 rounded-lg">
                            <Ionicons name="shield-checkmark" size={16} color="#f97316" />
                            <Text className="text-xs ml-1.5 mr-2 font-semibold text-orange-500">Proxy</Text>
                        </View>
                    </View>
                    ...
                    <View className="p-4 bg-gray-900">
                        <View className="flex-row items-center mb-3">
                            <View className="flex-1">
                                <Text className="text-white text-xl font-bold mb-1">{selectedChannel?.name}</Text>
                                <Text className="text-gray-400 text-sm">LCN {selectedChannel?.lcn} {selectedChannel?.language?.name}</Text>
                            </View>
                            <View className="flex-row items-center">
                                <View className="bg-orange-50020 px-2 py-1 rounded mr-2">
                                    <Text className="text-orange-500 text-xs font-semibold">LCN {selectedChannel?.lcn}</Text>
                                </View>
                                <Text className="text-gray-400 text-sm">{selectedChannel?.language?.name}</Text>
                                <Text className="text-gray-600 mx-2">|</Text>
                                <Text className="text-gray-400 text-sm">{selectedChannel?.genre?.name}</Text>
                            </View>
                        </View>
                    ...
                    {getRecommendedChannels().length > 0 && (
                        <View className="p-4 bg-black">
                            <Text className="text-white text-lg font-bold mb-3">More {selectedChannel?.language?.name} Channels</Text>
                            <ScrollView>
                                {getRecommendedChannels().map((channel) => (
                                    ...
                                    >
                                        <View className="relative" style={{ width: 120, height: 90 }}>
                                            {channel.imageUrl ? (
                                                <Image source={{ uri: channel.imageUrl }} style={{ width: '100%', height: '100%' }} resizeMode="cover" />
                                            ) : (
                                                <View className="w-full h-full bg-gradient-to-br from-orange-600 to-orange-800 items-center justify-center">
                                                    <Ionicons name="tv" size={30} color="white" opacity={0.5} />
                                                </View>
                                            )}
                                            <View className="absolute bottom-2 left-2 bg-black80 px-2 py-0.5 rounded">
                                                <Text className="text-white font-bold text-xs">{channel.lcn}</Text>
                                            </View>
                                        </View>
                                        <View className="flex-1 p-3 justify-center">
                                            <Text className="text-white font-semibold text-base numberOfLines={2}">{channel.name}</Text>
                                            <Text className="text-gray-400 text-xs mt-1">{channel.genre?.name} {channel.language?.name}</Text>
                                        </View>
            ...
            <Modal visible={showUserInfo} animationType="slide" transparent={true} onRequestClose={() => setShowUserInfo(false)}>
                <View className="flex-1 bg-black70 justify-end">
                        ...
                        <ScrollView className="px-6 py-4" style={{ maxHeight: 500 }}>
                            <View className="bg-gray-800 rounded-xl p-4 mb-4">
                                ...
                                <View className="border-t border-gray-700 pt-3 space-y-2">
                                    <View className="flex-row justify-between py-2">
                                        ...
                                        <Text className="text-white font-semibold">{user.packageName} Multi-Package</Text>
                                    </View>
                                    <View className="flex-row justify-between py-2">
                                    ...
                                    {daysRemaining !== null && (
                                        <View className="flex-row justify-between py-2">
                                            ...
                                            <Text className={`font-bold ${daysRemaining <= 7 ? 'text-red-500' : 'text-green-500'}`}>{daysRemaining} days</Text>
                                        </View>
                                    )}
                                    <View className="flex-row justify-between py-2">
                            ...
                            <View className="bg-gray-800 rounded-xl p-4 mb-4">
                                <View className="flex-row items-center mb-3 pb-3 border-b border-gray-700">
                                    <View className="bg-blue-50020 p-2 rounded-full mr-3">
                                ...
                                <View className="flex-row justify-between items-center py-2 border-b border-gray-700">
                                    ...
                                    <Text className="text-white text-xs font-mono font-semibold">{user.macAddress || 'N/A'}</Text>
                                </View>
                                {user.deviceName && (
                                    <View className="flex-row justify-between items-center py-2 border-b border-gray-700">
                                        <Text className="text-gray-400 text-sm">Device Name</Text>
                                        <Text className="text-white text-sm font-semibold numberOfLines={1}">{user.deviceName}</Text>
                                    </View>
                                )}
                                {user.modelName && (
                                    <View className="flex-row justify-between items-center py-2 border-b border-gray-700">
                                        <Text className="text-gray-400 text-sm">Model</Text>
                                        <Text className="text-white text-sm font-semibold numberOfLines={1}">{user.modelName}</Text>
                                    </View>
                                )}
                                {user.osName && user.osVersion && (
                                    <View className="flex-row justify-between items-center py-2 border-b border-gray-700">
                                        <Text className="text-gray-400 text-sm">Operating System</Text>
                                        <Text className="text-white text-sm font-semibold">{user.osName} {user.osVersion}</Text>
                                    </View>
                                )}
                                {user.deviceType && (
                                    ...
                                    {packagesList.map((pkg, index) => (
                                        <View key={index} className="bg-gray-800 rounded-xl p-4 mb-2">
                                            ...
                                            <View className="flex-row items-center mt-2">
                                                ...
                                                <Text className="text-gray-400 text-sm ml-2">{pkg.channelCount} channels</Text>
                                            </View>
                                        </View>
                                    ))}
                                </View>
                            )}
                            <TouchableOpacity className="bg-red-600 py-4 rounded-xl items-center mb-4" onPress={handleLogout}>
                                <View className="flex-row items-center">
